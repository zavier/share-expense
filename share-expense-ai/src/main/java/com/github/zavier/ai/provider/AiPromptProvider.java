...
public class AiPromptProvider {
    ...
    public String getChatSystemPrompt() {
        return """
            ...
            **⚠️ 最重要规则：工具调用是唯一执行操作的方式**
            ...
            5. **自我修复**: 当发现聊天记录和实际通过工具获取的数据不一致时，要有自动纠错的能力，尽量调用工具对异常数据进行补偿
    ...
    public String getSuggestionPrompt() {
        String basePrompt = """
            你是一个费用分摊记账智能建议助手。请根据用户历史对话消息的内容，预判用户可能要回复发送给你的内容
            ...
            对于用户的对话，要参考系统已有的功能，以及对话上下文的实际情况进行合理的回复
            如果涉及使用系统功能，需要在系统能力支持的前提下进行回复
            """;
